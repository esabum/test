...
class Zebra_Session {
     ...
     *  @return void
     */
    function __construct($session_name, $security_code, $session_lifetime = '', $lock_to_user_agent = true, $lock_to_ip = false, $gc_probability = '', $gc_divisor = '', $table_name = 'isession_data', $lock_timeout = 60) {
$ini_array = parse_ini_file(APPROOT . '/.config.ini', true);
        $host = $ini_array["mysql"]["mysqli_host"];
        $username = $ini_array["mysql"]["mysqli_user"];
        $password = $ini_array["mysql"]["mysqli_pass"];
        $database = $ini_array["mysql"]["mysqli_dbas"];

        require_once APPROOT . '/model/clsCrypt.php';
        $objCrypt = new Crypt();
        $password = trim($objCrypt->decrypt(rawurldecode($password)));

        $this->link = mysqli_connect($host, $username, $password, $database) or die('Could not connect to database!');
    ...
    private function _mysql_error() {
        ...
        return 'Zebra_Session: X' . mysqli_error($this->link);
    }
